...
MOUSE_X = 0
MOUSE_Y = 0
MOUSE_LEFT = 0 # 1 for pressed
MOUSE_RIGHT = 0 # 1 for pressed
# im seperating the click events from the inpEVENTS because i dont want them to be scoped to an input state, which should be reserved for controllers and keyboards. in other words only one mouse at any time.
CLICK_LEFT = 0 # 1 for clicked event
CLICK_RIGHT = 0 # 1 for clicked event
...
def update(noquit=False):
    global MOUSE_X, MOUSE_Y, MOUSE_LEFT, MOUSE_RIGHT, CLICK_LEFT, CLICK_RIGHT
    for name in STATES.keys():
        STATES[name]["EVENTS"] = []

    CLICK_LEFT, CLICK_RIGHT = 0, 0
