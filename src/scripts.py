...
def evaluate_literals(cmd, reference, world, related=None, logfunc=print):
    ...
    for idx in range(len(cmd)):
        ...
        try:
            ...
            if token == "MOUSEOVER?":
                actor = a.get_actor(reference)
                cmd[idx] = actor.collidepoint((inputs.MOUSE_X, inputs.MOUSE_Y))
            if token == "None":
                cmd[idx] = None
            if token == "[]":
                cmd[idx] = []
            ...
            if token.startswith("inp"):
                inp = token[3:]
                if inp == "MOUSE_X":
                    cmd[idx] = inputs.MOUSE_X
                elif inp == "MOUSE_Y":
                    cmd[idx] = inputs.MOUSE_Y
                elif inp == "MOUSE_LEFT":
                    cmd[idx] = inputs.MOUSE_LEFT
                elif inp == "MOUSE_RIGHT":
                    cmd[idx] = inputs.MOUSE_RIGHT
                elif inp == "CLICK_LEFT":
                    cmd[idx] = inputs.CLICK_LEFT
                elif inp == "CLICK_RIGHT":
                    cmd[idx] = inputs.CLICK_RIGHT
                elif a.get_actor(reference)._input_name is None:
